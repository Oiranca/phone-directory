...
export const registerSettingsIpc = (service: AppDataService, handle: IpcMain["handle"]) => {
  handle(CHANNELS.save, async (_event, payload: EditableAppSettings) => {
    const saved = await service.saveSettings(payload);
    return service.toEditableSettings(saved);
  });

  handle(CHANNELS.defaults, () => service.getEditableSettingsDefaults());

  handle(CHANNELS.browsePath, async (event, type: unknown): Promise<string | null> => {
