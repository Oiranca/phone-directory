/**
 * Integration tests for the full beepers (pager) data pipeline — OIR-270.
 *
 * Exercises the REAL code paths end-to-end (no mocking of the functions
 * under test) across the three stacked features that make up the beepers
 * pipeline:
 *
 ...
 *       "Busca 1"/"Busca 2" columns, parsed via parseBeeperSheets into
 *       ImportedBeeperRecords for the separate beepers.json store.
 ...
 *       in-memory-built .xlsx workbook), asserting beeper updates apply,
 *       an unrelated contact's beeper is left untouched, and no beeper-only
...
// ---------------------------------------------------------------------------
// (a) OIR-265 — Agenda-tabular sheet row -> real ContactRecord.beepers +
//     corporativo-kind phone entry
// ---------------------------------------------------------------------------

describe("beepers pipeline — (a) Agenda-tabular row through the real import conversion (OIR-265)", () => {
  const AGENDA_HEADER_ROW_WITH_BEEPER_CORPORATIVO = [
    "Nombre",
    "Categoría",
    "Servicio",
    "Número 1",
  ...
  it("carries a synthetic 'Busca 1' + 'Corporativo 1' row all the way to a real ContactRecord.beepers entry and a corporativo-kind phone", async () => {
    const row = [
      "Ana Pérez", // Nombre
    ...
    const sheet = makeSheet("Urgencias", [AGENDA_HEADER_ROW_WITH_BEEPER_CORPORATIVO, row]);
    ...
    const rawBeepers = JSON.parse(rows[0]!.beepers!) as Array<{ number: string; label?: string }>;
    expect(rawBeepers).toEqual([{ number: "1111" }]);
    ...
    // The beeper (pager) code lands on the contact's own `beepers` array.
    expect(record.beepers).toEqual([{ number: "1111" }]);
...
// ---------------------------------------------------------------------------
// (b) OIR-266 — dedicated "Buscas Todos"-style sheet -> ImportedBeeperRecords
// ---------------------------------------------------------------------------

describe("beepers pipeline — (b) dedicated 'Buscas Todos' sheet through the real pager-sheet parser (OIR-266)", () => {
  it("parses a synthetic 'Buscas Todos' sheet into correct ImportedBeeperRecords, one per non-empty Busca cell", () => {
    const sheet = {
      name: "Buscas Todos",
      rows: [
        [
        ...
        // Row with a department-only Beeper (no named holder).
    ...
    const result = parseBeeperSheets([sheet]);
...
describe("beepers pipeline — (c) full-agenda reimport through AppDataService (OIR-267)", () => {
  let testRoot: string;
  ...
  /** Two-contact Agenda-tabular sheet: Ana carries `anaBeeper`, Luis carries "2222" unchanged across imports. */
  const buildAgendaRows = (anaBeeper: string) => [
    AGENDA_HEADER_ROW_WITH_BEEPER,
    [
  ...
  beforeEach(async () => {
    testRoot = await fs.mkdtemp(path.join(os.tmpdir(), "phone-directory-beepers-pipeline-"));
  ...
  it("applies a real beeper update, preserves an unrelated contact's unchanged beeper, and never surfaces a beeper-only difference as a conflict", async () => {
    ...
    const firstSourcePath = writeWorkbook(path.join(testRoot, "incoming"), "agenda-beepers-1.xlsx", [
    ...
    expect(anaFirst.beepers).toEqual([{ number: "1111" }]);
    expect(luisFirst.beepers).toEqual([{ number: "2222" }]);

    // --- Reimport: Ana's beeper changes to 9999, Luis's row is unchanged ---
    const secondSourcePath = writeWorkbook(path.join(testRoot, "incoming"), "agenda-beepers-2.xlsx", [
      { name: "Urgencias", data: buildAgendaRows("9999") }
    ]);

    const preview = await service.previewCsvImport(secondSourcePath);

    // A beeper-only difference must never be surfaced as a user-facing
    // conflict — Ana's row differs only in `beepers`.
    expect(preview.conflictCount).toBe(0);
    expect(preview.conflictedRecords).toEqual([]);
    // Ana's row is a real (beeper) update; Luis's row is byte-for-byte
    ...
    // Ana's beeper genuinely updated...
    expect(anaAfter.beepers).toEqual([{ number: "9999" }]);
    expect(anaAfter.audit.updatedAt).not.toBe(anaFirst.audit.updatedAt);

    // ...while Luis's beeper (and audit trail) is left completely untouched.
    expect(luisAfter.beepers).toEqual([{ number: "2222" }]);
    expect(luisAfter.audit.updatedAt).toBe(luisFirst.audit.updatedAt);
  });

  it("preserves an existing beeper (does not wipe it) when a later plain reimport row has no beeper column data at all", async () => {
    const { AppDataService } = await import("./app-data.service.js");
    ...
    const firstSourcePath = writeWorkbook(path.join(testRoot, "incoming"), "agenda-beepers-plain-1.xlsx", [
      { name: "Urgencias", data: buildAgendaRows("1111") }
    ]);
    const firstResult = await service.importCsvDataset(firstSourcePath);
    const anaFirst = firstResult.contacts.records.find((record) => record.displayName === "Ana Pérez")!;
    ...
    const plainCsvPath = path.join(testRoot, "incoming", "agenda-beepers-plain-2.csv");
    ...
    // beepers must never appear as a diffable field on the conflict summary
